...
class Model(nn.Module):
    def __init__(self, opt):
        ...
        ########### InterFormer
        self.multimodal_transformer = Transformer(embedding_dim=basic_dims, depth=depth, heads=num_heads, mlp_dim=basic_dims, n_levels=num_modals)
        self.multimodal_decode_conv = nn.Conv2d(transformer_basic_dims*num_modals, transformer_basic_dims*num_modals, kernel_size=1, padding=0)
        ########### InterFormer
    ...
    def forward(self, modal1, modal2,is_training):
        ...
        ########### IntraFormer
        modal1_token_x3 = self.modal1_Linear(modal1_x3.reshape(modal1.shape[0], basic_dims*8, -1))  ###Face: N * 128* 16
        modal2_token_x3 = self.modal2_Linear(modal2_x3.reshape(modal2.shape[0], basic_dims*8, -1))  ###EEG: N * 128* 16

        modal1_intra_token_x3 = self.modal1_transformer(modal1_token_x3, self.modal1_pos)
        modal2_intra_token_x3 = self.modal2_transformer(modal2_token_x3, self.modal2_pos)

        modal1_intra_x3 = modal1_intra_token_x3.reshape(modal1_x3.shape[0], modal1_x3.shape[1], modal1_x3.shape[2], modal1_x3.shape[3])
        modal2_intra_x3 = modal2_intra_token_x3.reshape(modal2_x3.shape[0], modal2_x3.shape[1], modal2_x3.shape[2], modal2_x3.shape[3])
        ...
        x1 = torch.cat((modal1_x1, modal2_x1), dim=1) #####B*64*16*16
        x2 = torch.cat((modal1_x2, modal2_x2), dim=1)  #####B*128*8*8
        x3_inter = torch.cat((modal1_intra_x3, modal2_intra_x3), dim=1) #####B*256*4*4

        ########### InterFormer
        multimodal_pos = torch.cat((self.modal1_pos, self.modal2_pos), dim=1)   ###1*256*16
        multimodal_inter_token_x3 = self.multimodal_transformer(x3_inter.reshape(x3_inter.shape[0], x3_inter.shape[1], -1), multimodal_pos) ###B*256*16
        multimodal_inter_x3 = self.multimodal_decode_conv(multimodal_inter_token_x3.reshape(multimodal_inter_token_x3.shape[0], transformer_basic_dims*num_modals, patch_size, patch_size)) ###B*256*4*4
        x3_inter = multimodal_inter_x3

        fuse_pred, preds = self.decoder_fuse(x1, x2, x3_inter)
        ########### InterFormer
        
        if is_training:
            return fuse_pred, (modal1_pred, modal2_pred), (modal1_L2, modal2_L2), preds
        return fuse_pred
